...
def _encode_text(key, text):
    ...
    final_encoding = (
        tree_encoding
        + "{0:08b}".format(8)
        + "{0:08b}".format(bits_less_for_8bit_code)
        + encoded_text_code
    )
...
def encode(text="", key=0, path=None, inplace=False):
    ...
    Input Parametes
    ---------------
        * text: Text that needs to be encoded. {Default=''} (Optional)
...
def decode(text="", key=0, path=None, inplace=False):
    ...
    Input Parametes
    ---------------
        * text: Text that needs to be decoded. {Default=''} (Optional)
        
        * key: If secret key was used to embedd to this text. 
            {Default=0} (Optional)
